...
export default function SingleTv() {
    ...
    const single_movie_url = `https://api.themoviedb.org/3/tv/${id}?api_key=${import.meta.env.VITE_API_KEY}`
    const movie_cast_url = `https://api.themoviedb.org/3/tv/${id}/credits?api_key=${import.meta.env.VITE_API_KEY}`
    const [genres, setGenres] = useState([])

    useEffect(() => {
        setIsLoading(true)
        fetch(single_movie_url)
            .then(res => res.json())
            .then(data => {
                setMovieData(data)
        ...
        <div className="bg-dark">
            <div className="container">
                <div className="card border-0 py-5 bg-dark">
                    {(movieData && genres) && <div className="row g-0">
